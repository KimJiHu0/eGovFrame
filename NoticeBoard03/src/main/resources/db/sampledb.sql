...
select rownum from SAMPLE where rownum between 1 and 10; -- o
select * from SAMPLE where rownum between 11 and 20; -- x
-- 순서대로 rownum이 메겨지지만 1~10 까지는 되고, 11 ~ 20까진 안돼.

select * from 
	( 
		select rownum as rn, id, name, description, use_yn, reg_user
		from sample
	)
